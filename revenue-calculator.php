<?php
/**
 * /revenue-calculator: homepage mirror + AOV/revenue calculator + lead form.
 *
 * Every section below this comment is the homepage, copied as-is, with
 * exactly two additions layered in:
 *
 *   1. The "Why Brix" section's right-hand visual (the before/after cart
 *      comparison on the homepage) is replaced with a calculator: AOV,
 *      monthly revenue and ad spend in, an estimated missing-revenue
 *      figure out. See js/calculator.js for the math.
 *   2. A lead-capture section sits directly after "Why Brix", same
 *      pattern as the form on /cart-that-sells and /walkthrough
 *      (includes/lead-form.php), tagged 'revenue-calculator' so these
 *      leads are told apart from the others in the admin. The
 *      homepage's Hero (with the cart-mock demo) is unchanged but now
 *      runs third, after the calculator and the form rather than first.
 *
 * Unindexed for now, same reasoning as the other campaign landing pages:
 * this is a near-duplicate of the homepage, and a search engine should
 * never have to choose between the two. Flip $page_robots once this has
 * a real destination (an ad campaign, an email) to send traffic to.
 */
...
$lead = brix_lead_handle('revenue-calculator', [
    'ad_spend' => 'Monthly ad spend',
]);

$page_title       = 'Brix – See the Revenue Your Shopify Cart Is Missing';
$page_description = 'Estimate the extra revenue a Shopify cart upsell app like Brix can add to your store, then talk to us about your own numbers. Free plan available.';
$page_canonical   = 'revenue-calculator';
$page_robots      = 'noindex, nofollow, noarchive, nosnippet, noimageindex';
$page_nav         = null;
$footer_col3      = 'case-studies';
$page_scripts     = '<script src="/js/calculator.js?v=' . ASSET_CALC_VER . '"></script>';
...
<section class="section" id="why-brix">
  <div class="container f-grid">
    ...
    <!-- Signature: the AOV / revenue calculator, in place of the homepage's
         before/after cart comparison. Logic lives in js/calculator.js,
         guarded on #aovCalc so it never runs anywhere else. -->
    <div class="wb-visual">
      <div class="wb-glow" aria-hidden="true"></div>
      <div class="calc-card reveal" style="--d:.1s" id="aovCalc">
        <div class="calc-head">
          <span class="calc-badge">Free calculator</span>
          <h3>What is your cart leaving on the table?</h3>
          <p class="calc-sub">Brix merchants typically see a 17–20% lift in AOV. See what that means in dollars for your store.</p>
        </div>

        <div class="calc-field">
          <label for="calcCurrency">Currency</label>
          <select id="calcCurrency" class="calc-select">
        ...
        <div class="calc-field">
          <label for="calcAov">Average order value</label>
          <div class="calc-input"><span class="calc-currency-sym">$</span><input type="number" id="calcAov" inputmode="decimal" min="0" step="0.01" placeholder="65"></div>
        </div>
        <div class="calc-field">
          <label for="calcRevenue">Monthly revenue</label>
          <div class="calc-input"><span class="calc-currency-sym">$</span><input type="number" id="calcRevenue" inputmode="decimal" min="0" step="1" placeholder="40000"></div>
        </div>
        <div class="calc-field">
          <label for="calcAdSpend">Monthly ad spend <span>optional</span></label>
          <div class="calc-input"><span class="calc-currency-sym">$</span><input type="number" id="calcAdSpend" inputmode="decimal" min="0" step="1" placeholder="6000"></div>
        </div>

        <button class="btn btn-primary calc-btn" type="button" id="calcBtn">Show my missing revenue</button>

        <div class="calc-result" id="calcResult" hidden>
          <p class="calc-result-label">You could be missing</p>
          <p class="calc-result-fig"><span id="calcMissing">$0</span><small>/month</small></p>
          <div class="calc-result-row"><span>New AOV</span><b id="calcNewAov">$0</b></div>
          <div class="calc-result-row"><span>New monthly revenue</span><b id="calcNewRevenue">$0</b></div>
          <p class="calc-result-note" id="calcAdNote"></p>
        </div>
      </div>
      <p class="ba-pict-cap">Based on Brix merchants’ typical <b>+17–20% AOV</b> lift</p>
    </div>
  </div>
</section>

<!-- ============ THE FORM ============ -->
<!-- Second section, directly under "Why Brix": the calculator above makes
     the case in the visitor's own numbers, and the ask follows immediately,
     before the page has spent any of their patience. The homepage's Hero
     (with the cart-mock demo) now runs third, below this. Same form, same
     table and same admin row as /cart-that-sells and /walkthrough
     (includes/lead-form.php), tagged 'revenue-calculator'. -->
<section class="section dm-form-sec" id="get-the-numbers">
  <div class="container">
    <div class="dm-form-grid">
      <div class="dm-form-copy reveal">
        <p class="eyebrow">Want the full picture?</p>
        <h2>We’ll run the numbers on your actual store</h2>
        <p class="dm-form-sub">The calculator above is a quick estimate. Tell us about your store and we’ll come back within one business day with what we’d turn on first, and roughly what it’s worth for you specifically.</p>
...
<?php if ($lead['sent']): ?>
        <div class="dm-form-done">
          ...
          <p>We have your details and will come back within one business day. If you’d rather not wait, Brix is free to install right now.</p>
          <a class="btn btn-primary" href="<?= e(SHOPIFY_APP_URL) ?>" target="_blank" rel="noopener">Install free on Shopify</a>
        </div>
<?php else: ?>
        <form class="dm-form" method="post" action="#get-the-numbers" novalidate>
          <?= csrf_field() ?>
          <input type="hidden" name="t" value="<?= time() ?>">
          <?php /* Honeypot: never shown, never filled by a person. */ ?>
          <div class="dm-hp" aria-hidden="true">
            <label for="rc-website">Website</label>
            <input type="text" id="rc-website" name="website" tabindex="-1" autocomplete="off">
          </div>
...
<?php endif; ?>
          ...
          <div class="dm-field">
            <label for="rc-store">Store URL <span>optional</span></label>
            <input type="text" id="rc-store" name="store_url" autocomplete="url"
                   placeholder="yourstore.com" value="<?= e($lead['values']['store_url']) ?>">
          </div>

          <div class="dm-field">
            <label for="rc-adspend">Monthly ad spend <span>optional</span></label>
            <select id="rc-adspend" name="ad_spend">
...
<!-- ============ HERO ============ -->
<!-- Third section now: the homepage's hero, unchanged, sits after the
     calculator and the form instead of leading the page. -->
<section class="hero">
