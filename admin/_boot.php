 ...
 * SQL. Once per session: the check is two cheap queries, but there is
 * no reason to repeat it on every page view.
 */
if (brix_is_configured()) {
    brix_session_start();
}

if (brix_is_configured() && empty($_SESSION['brix_schema_checked'])) {
    try {
        brix_upgrade_schema(db());
        $_SESSION['brix_schema_checked'] = true;
    } catch (Throwable) {
