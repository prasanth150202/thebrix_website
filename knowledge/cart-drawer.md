...
Forge bundle pages are separate and can each have their own design.

## Cart Drawer Settings Reference

The Cart Drawer editor is organized into a left-hand section list. Each section edits one
part of the drawer, shown live in the right-hand preview.

| Section | Controls |
|---|---|
| Design | Cart width, corner rounding, drop shadow, opening animation |
| General | Auto-open on add, open on icon click, drawer position (left/right) |
| Header Style | Title text, close-button style, header colors, bottom border |
| Announcements | Promotional strip text, colors, font size |
| Progress Bar | Mode, position, tiers, reward products, colors, completion message, confetti |
| Coupon Slider | Style template, layout, alignment, featured coupons, per-coupon appearance and timers |
| Upsell Products | AI vs. manual mode, rules, limit, position, layout, title/button text |
| Empty Cart | Empty message, continue-shopping button, product recommendations |
| Checkout Button | Button text, footer text, colors, corner rounding, mobile swipe mode |
| Custom CSS | Freeform CSS override field; watermark toggle |
