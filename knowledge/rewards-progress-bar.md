...
You choose whether progress is measured by amount spent or by number of items, where the
bar sits in the drawer (top, or below the cart items), and whether it's visible even when
the cart is empty. Bar colors (background, fill, icon, and message) are fully customizable,
and you can set a custom completion message with an optional confetti celebration when
every tier is unlocked.

**Example:** Tier 1 at ₹500 unlocks a free sample; Tier 2 at ₹1,200 unlocks free shipping
...
1. Open Cart Drawer → Progress Bar and toggle it on.
2. Choose whether tiers are measured by amount spent or item count, and set the bar's
   position.
...
**Reward products aren't being added at the right tier:** Double-check the tier's minimum
spend/count and confirm reward products were selected for that specific tier (each tier has
its own product picker).
