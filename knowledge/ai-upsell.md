...
Additional controls: show upsells even on an empty cart, keep showing an offer even after
the shopper already added it, display product reviews on upsell cards, position
(top/bottom of the item list), layout (carousel or 2-column grid), direction, title text
and color, and button text.
...
button, and where it's pinned on the product page.

**Note on recommendation scope:** Cart Upsell recommends based on what's currently in the
cart; FBT recommends based on the product page being viewed; Combo Forge's AI suggestions
help shoppers complete a bundle. Each can run in AI mode or manual-rules mode
independently.

## Set Up Frequently Bought Together

1. Open FBT from the main navigation.
2. Choose a visual template (Classic Grid, Modern Cards, or Vertical List) and an
   interaction style (Classic or Quick Add).
3. Choose AI mode (automatic pairing) or Manual mode, then define which products trigger
   which recommendations.
4. Set widget placement on the product page and toggle whether prices and an "Add all"
   button are shown.
5. Save and confirm the FBT app embed block is active in your theme editor.
